...
{
    export class PostContainer extends React.Component<PostPropState.Props, PostPropState.State>
    {
        ...
        private currentPostId: string | undefined = undefined;
...
// {commentId: "1", postId: "7", postUserId: "1", commentUserId: "12", commentUserIcon: "https://uploads.dailydot.com/2018/10/olli-the-polite-cat.jpg?q=65&w=800&ar=2:1&fit=crop", name: "asd asd", content: "yolo", media: "1672725423794-870314175.png".split(",").map(m => `${process.env.REACT_APP_API_URL!}/media/${m}`), time: "3-1-2023/13:57"}
        ...
        componentDidUpdate(prevProps: Readonly<PostPropState.Props>, prevState: Readonly<PostPropState.State>, snapshot?: any): void 
        {
            ...
            if(this.currentPostId !== undefined && this.currentPostId !== "" && this.props.postId !== undefined && this.props.postId !== "")
            {
                if(this.currentPostId?.toString() !== this.props.postId.toString())
                {
                        this.currentPostId = this.props.postId;
        ...
        async fetchPost(): Promise<void> 
        {
            ...
            const key: string = encodeURIComponent(this.props.cookie.sconnecti.toString());
            const postId: string = this.props.postId.toString();

            const promise = await fetch(`${process.env.REACT_APP_API_URL}/post?u=${key}&p=${postId}`);

            this.setState(() => ({ postLoader: true }));

            if(promise.ok)
            {
                const result: { success: boolean, data: Array<any>, result: string, friendStatus: any } = await promise.json();
                ...
                if(result.success)
                {
                    if(result.data.length !== 0)
                    {
                        ...
                        if(result.friendStatus.length !== 0)
                        {
                            const action: string = result.friendStatus[0].action;
                            const status: string = result.friendStatus[0].friendStatus;

                            this.setState(() => ({ post: newPostData, friendAction: action, friendStatus: status }));

                        } else 
                        {
                            this.setState(() => ({ post: newPostData, friendAction: "ok", friendStatus: "ok" }));
                        }
                ...
                <div className="content-container-post-only">
                    ...
                    <div className="delete-post-confrim-block">
                        <div className="delete-post-block">
                            <div>
                                <span>
                                    {this.state.deleteConConfirm.userId === this.userId?.toString() ?
                                    "Delete this comment?"
                                    :
                                    "Hide this comment?"
                                    }
                    ...
                    <div className="post-form-input-container" ref={this.postFormContainer} onPointerUp={(e) => {
                        ...
                        <div className="post-result-container">
                            ...
                            <div className="post-result-body">
                                <div>
                                    <span>Please choose a file size less than 5 megebyte</span>
                                    <span>For each of the image/video file</span>
                                </div>
                            </div>
                        </div>
...
{/* 
                            ...
                            !post.deleted ?
                                this.state.friendStatus === "friended" || this.state.friendStatus === "none" || this.state.friendStatus === "ok" ?
                                <div className="post-container" key={`post-only-${post.postId}-${i}`}>
                                    <div className="post-block">
                                        <div className="post-header">
                                            ...
                                            <div className="post-header-option-container">
                                                ...
                                                {post.controlPost ?
                                                post.userId.toString() === this.userId?.toString() ?
                                                <div className="post-control-block-user">
                                                    <div onClick={() => this.deletePost(post.postId, i, post.userId.toString())}>Delete</div>
                                                </div>
                                                :
                                                <div className="post-control-block-user">
                                                    <div onClick={() => this.deletePost(post.postId, i, post.userId.toString())}>Hide</div>
                                                    {/* {post.reported ? 
                                                    <div>Reported</div>
                                                    :
                                        ...
                                        <div className="post-media-container">
                                            {post.media.map((media, index, array) => (
                                                ...
                                                <div className="post-media-block" key={`post-${post.postId}-${index}`}>
                                                ...
                                                <div className="post-media-block-p" key={`post-${post.postId}-${index}`}>
                                                    {(media.split('.')[media.split(".").length - 1] === "png" || media.split('.')[media.split(".").length - 1] === "jpg" || media.split('.')[media.split(".").length - 1] === "jpeg" || media.split('.')[media.split(".").length - 1] === "gif" || media.split('.')[media.split(".").length - 1] === "tiff") ?
                                                ...
                                                :
                                                (index > 0 && index <= 2 && array.length > 3) ?
                                                <div className="post-media-block-p" key={`post-${post.postId}-${index}`} onPointerUp={() => this.viewImgContainer(array, index)}>
                                                    {(media.split('.')[media.split(".").length - 1] === "png" || media.split('.')[media.split(".").length - 1] === "jpg" || media.split('.')[media.split(".").length - 1] === "jpeg" || media.split('.')[media.split(".").length - 1] === "gif" || media.split('.')[media.split(".").length - 1] === "tiff") ?
                                            ...
                                            <div className="comment-input-block">
                                                ...
                                                <div className="input-content-container">
                                                    ...
                                                    <div className="input-content-option-container">
                                                        ...
                                                        <div>
                                                            <label htmlFor={`commentOnlyMedia${i}`}>
                                                            ...
                                                            <input type="file" hidden={true} id={`commentOnlyMedia${i}`} ref={post.commentFile} accept="image/*, video/*" onChange={() => this.commentBlob(post.commentFile ,i)} />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="comment-section-container">
                                            ...
                                            <div className="comment-block" key={`c-only-${comments.commentId}-${commentsIndex}`}>
                                                ...
                                                <div className="comment-box">
                                                    ...
                                                    <div className="comment-user-media-block">
                                                        {comments.media.map((m, i, a) => (
                                                            ...
                                                            <img src={`${process.env.REACT_APP_API_URL}/media/${m}`} key={`comment-${comments.id}-${i}`} onPointerUp={() => this.viewImgContainer(a, i)} alt={`${process.env.REACT_APP_API_URL}/media/${m}`}/>
                                                            :
                                                            <video src={`${process.env.REACT_APP_API_URL}/media/${m}`} controls key={`comment-${comments.id}-${i}`}></video>
                                                        ))}
                                                    </div>
                                                    :
                                                    null
                                                ...
                                                <div className="comment-option">
                                                    ...
                                                    {comments.control ?
                                                    comments.commentUserId.toString() === this.userId?.toString() ?
                                                    <div className="comment-control-block-user">
                                                        {comments.deleted ?
                                                        <div>Deleted</div>
                                                        :
                                                        <div onClick={() => this.deleteComment(comments.commentId, commentsIndex, i, post.postId, comments.commentUserId.toString())}>Delete</div>
                                                        }
                                                    </div>
                                                    :
                                                    <div className="comment-control-block-user">
                                                        ...
                                                        <div>Hided</div>
                                                        :
                                                        <div onClick={() => this.deleteComment(comments.commentId, commentsIndex, i, post.postId, comments.commentUserId.toString())}>Hide</div>
                                                        }
                                ...
                                :
                                <div className="not-found-post-container" key={`post-only-${post.postId}-${i}`}>
                                    {this.state.friendAction === "blocking" ?
                                    <span>You can't see this post, because you blocked this user.</span>
                                    :
                                    <span>You can't see this post, because the user of this post has blocked you.</span>
                                    }
                                </div>
                            :
                            <div className="not-found-post-container" key={`post-only-${post.postId}-${i}`}>
                            ...
                            {this.state.noMorePost && this.state.post.length !== 0 ? 
                            <div className="not-found-post-container">
                                <span>404 post not found...</span>
                            </div>
                            :
    ...
    export function Container(): JSX.Element
    {
        ...
        let [checkPostId, setCheckPostId]: any = useState();
        ...
        useEffect((): void =>
        {
            if(!LOAD)
            {
                ...
                {
                    setCheckPostId(checkPostId = postId.toString());

                } else 
                {
                    setCheckPostId(checkPostId = undefined);
                }
            }
        ...
        return (
            checkPostId !== undefined && checkPostId !== "" ?
            <PostContainer Navigate={navigate} cookie={cookie} setCookie={setCookies} removeCookie={removeCookie} Location={location} postId={postId}/>
            :
            <div className="not-found-post-container">
                <span>404 post not found...</span>
            </div>
